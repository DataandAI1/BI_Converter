...
const SQL_HEADER = (fqn: string): string[] => [
  `-- Generated by BI_Converter — Databricks AI/BI semantic layer (Tableau → Databricks)`,
...
function emitTableView(
  ...
  const unresolved = notes.review.length > reviewCountBefore;
  const lines = [
    ...SQL_HEADER(ds.fqn),
    // The binding resolver could not turn this Tableau reference into a full Unity
    // Catalog name, and refused to guess one (spec §6.3). The view still ships, so the
    // pack stays complete and reviewable, but it says what a human has to decide.
    ...(unresolved
      ? [`-- TODO: unresolved source for ${fromRef} — supply the full catalog.schema.object`]
      : []),
...
function emitMetricView(
  ...
  const lines: string[] = [];
  lines.push(`# Generated by BI_Converter — Databricks AI/BI semantic layer (Tableau → Databricks)`);
 ...
 *  embedded `.hyper` extract, or a connection the converter could not resolve to a Unity Catalog
...
"""Extract-rescue script — BI_Converter Databricks AI/BI semantic layer pack.

For a Tableau datasource with no matched source system (an embedded .hyper extract, or
a connection the converter could not resolve to a Unity Catalog name), the upstream database
